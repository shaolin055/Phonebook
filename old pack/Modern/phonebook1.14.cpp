/*------------------------- 2,3,8,20,21,34-36,49,83,343-362---------------------------*/
/*------------------------------------1.14--------------------------------------*/
#include<stdio.h>
#include<conio.h>
#include<string.h>
...
void edit1();
/*------------------------------------------------------------------------------*/
char file[60]  = "F:\\Phonebook\\data\\phone.fat";
char up[60]    = "f:\\Phonebook\\upgrade.fat";
char file1[60]  ="F:\\Phonebook\\data\\personal.fat";
...
{  int i,j,k=1,m=1,n=1;
	...
	{       j=strcmp(name,p[i].na);
			  k=strcmp(phone,p[i].ph);
		if(j==0&&strcmp(name,"\0")) {      m=2;printf("\n\t\t\tSEARCHING BAISED ON NAME:\n **  name -> %s\n     phone -> %s\n     address\e-mail -> %s\n",p[i].na,p[i].ph,p[i].ad);}
		if(k==0&&strcmp(phone,"\0")) {     n=2;printf("\n\t\t\tSEARCHING BAISED ON PHONE:\n     name -> %s\n **  phone -> %s\n     address\e-mail -> %s\n",p[i].na,p[i].ph,p[i].ad);}
										 }
...
coming soon... : multiple erase option
*/
